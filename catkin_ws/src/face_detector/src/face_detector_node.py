...
import time
import threading
class face_detector_node(object):
	def __init__(self):
		...
		self.pub_car_cmd = rospy.Publisher("/hbc/face_detecteor_node/car_cmd",Twist2DStamped,queue_size=1)

		# To do : publish ros message topic: /node_name/image_with_face, datatype: Image 
		self.pub_image_face = rospy.Publisher("/hbc/face_detector_node/image_with_face", Image, queue_size=1)
		...
		self.sub_image_origin = rospy.Subscriber("/hbc/face_detector_node/image", CompressedImage, self.cbImage, queue_size=1)
	...
	def cbFacedetect(self, image_msg):		
		...
		faceCascade = cv2.CascadeClassifier('/home/ubuntu/duckietown/catkin_ws/src/haarcascade_frontalface_default.xml')
...
if __name__ == "__main__":
	...
	face_detector_node = face_detector_node()
	rospy.spin()